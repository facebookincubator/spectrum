...
enum class Crop
constructor(val title: String, val description: String?, val requirement: CropRequirement?) {
  NO_CROP("No crop", null, null),
  CROP_1(
      "Absolute crop 1",
...
enum class Resize constructor(val title: String, val requirement: ResizeRequirement?) {
  NO_RESIZE("No resize", null),
  EXACT_2X2("Exact 2x2", ResizeRequirement(Mode.EXACT, ImageSize(2, 2))),
  EXACT_200X200("Exact 200x200", ResizeRequirement(Mode.EXACT, ImageSize(200, 200))),
  EXACT_1000X1000("Exact 1000x1000", ResizeRequirement(Mode.EXACT, ImageSize(1000, 1000))),
  EXACT_OR_SMALLER_200X200(
      "ExactOrSmaller 200x200", ResizeRequirement(Mode.EXACT_OR_SMALLER, ImageSize(200, 200))),
  EXACT_OR_SMALLER_1000X1000(
      "ExactOrSmaller 1000x1000", ResizeRequirement(Mode.EXACT_OR_SMALLER, ImageSize(1000, 1000))),
}
...
constructor(val title: String, val description: String, val mode: EncodeRequirement.Mode) {
...
data class EncodeModeSpinnerEntry(val encodeMode: EncodeMode) :
    Entry(encodeMode.title, encodeMode.description) {
  companion object {
