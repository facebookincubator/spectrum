...
class ConfigurationActivity : AppCompatActivity() {
  ...
  private fun <T> createEnumParameterView(
    ...
    return layoutInflater.inflate(R.layout.configuration_group_parameter_enum, groupLayout, false)
        .also {
          ...
          it.findViewById<Spinner>(R.id.configuration_parameter_enum_spinner).also { spinner ->
            spinner.configureSimpleSpinner(layoutInflater, parameter.entries.toList()) {
              model.set(parameter, it)
            }
  ...
  private fun createBooleanParameterView(
      groupLayout: ViewGroup,
